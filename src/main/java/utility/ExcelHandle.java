...
public class ExcelHandle {
	...
	public static void writeUrlDataIntoExcelFile_Old(String filePath, LinkedHashMap<String, String[]> urlDetailsMap)
			throws IOException {
		try {
	...
	public static void writeUrlDataIntoExcelFile(String filePath, LinkedHashMap<String, String[]> urlDetailsMap)
		...
		try {
			...
			for (int i = 1; i < noOfEntriesInTheMap + 1; i++) {
				XSSFRow row = workSheet.createRow(i);
				String[] urlDetailsArray = urlDetailsMap.get(ketList[i - 1]);
				row.createCell(0, CellType.STRING).setCellValue((String) urlDetailsArray[0]);
				row.createCell(1, CellType.STRING).setCellValue((String) urlDetailsArray[1]);
				row.createCell(2, CellType.STRING).setCellValue((String) urlDetailsArray[2]);
			}
			// Saving the details into the map
			FileOutputStream excelFile = new FileOutputStream(filePath);
			workbook.write(excelFile);
